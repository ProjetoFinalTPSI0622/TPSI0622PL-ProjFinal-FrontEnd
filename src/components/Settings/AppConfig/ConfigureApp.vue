<template>
  <ConfigGroup>
    ...
    <div class="flex flex-col mt-3">
      <SubTitle SubTitle="Criar Novo Estado" class="pl-5" />
  ...
  <ConfigGroup class="mt-5">
    ...
    <div class="bg-blue w-full h-px rounded my-3" />
    <SubTitle SubTitle="Criar Nova Categoria" class="pl-5" />
    <div class="mt-5 w-full flex flex-row gap-5 justify-between px-10">
      <div class="w-2/5">
...
const loadData = async () => {
  try {
    status.value = (await StatesService.getStates()).data;
...
const createCategory = () => {
  CategoriesService.createCategory({ name: catName.value });
};
